...
- [ ] **Finalize and freeze the fair-value rate.** Pull a final treasury snapshot (ICVC supply, treasury ICP + nICP, WaterNeuron `get_info.exchange_rate`), bake the constants via an upgrade, then leave them fixed. Inputs currently reflect the 2026-06-02 snapshot.
- [ ] **Remove the faucet.** Delete the `faucet` method, its cooldown state, the ~20M ICVC faucet pre-funding, and the faucet card in the SPA.
