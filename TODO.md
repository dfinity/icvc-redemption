...
- [ ] **Switch to the real ledgers.** Wired: the `prod` environment (separate from play `ic`, same network) points redemption at the production ICVC token (`m6xut-mqaaa-aaaaq-aadua-cai`) + NNS ICP ledger (`ryjl3-tyaaa-aaaaa-aaaba-cai`), resolved from the `prod` key in `canister_ids.json` into the init args + frontend `config.js` (`DO_LEDGERS=0`, no balance reseed). `getLedgers()` makes the wiring verifiable on-chain. **Remaining:** run the `-e prod` cutover from a fresh clone as the HSM (`MIGRATIONS.md` §7). Core step the rest depend on.
