...
**Prod is deployed** (2026-06-19): the real-value canisters (redemption `aujl2-pqaaa-aaaae-agwja-cai`, frontend `atino-ciaaa-aaaae-agwjq-cai`) run on the `prod` env against the real ICVC ledger (`m6xut-mqaaa-aaaaq-aadua-cai`) and NNS ICP ledger (`ryjl3-tyaaa-aaaaa-aaaba-cai`). The **faucet is removed** and sign-in is standard per-origin Internet Identity. The original **play** deployment (canisters `yofbu`/`zdlf2`, tagged `play-v1`) still runs against our own ICRC-1 ledger copies, *not* the production tokens. See `README.md` for the deployment table; pool funding is in progress.
...
- `bash scripts/deploy.sh -e ic` — **play** mainnet upgrade (existing `yofbu`/`zdlf2` canisters + our ICRC-1 ledger copies). Builds the redemption WASM reproducibly via `Dockerfile.build` + hash-gate; frontend syncs via the `@dfinity/asset-canister` recipe — needs Docker, no dfx
- `bash scripts/deploy.sh -e prod` — **real-value** deployment: a separate environment (same `ic` network, own mapping `prod.ids.json`) that creates fresh redemption + frontend canisters and wires them to the REAL ICVC (`m6xut-…`) + ICP (`ryjl3-…`) ledgers. HSM-run; see [`MIGRATIONS.md`](./MIGRATIONS.md) §7
...
- The two ledger ids are **install-time init args** (`icvc_ledger_id`/`icp_ledger_id`), not baked constants — so the *same* reproducible wasm serves local/play/prod and the hash doesn't encode the ledger wiring (local/test ledgers also get fresh ids each deploy, which can't be compile-time constants). `deploy.sh` resolves them per-environment from the registry (`canister_id <name>` → `.icp/data/mappings/<env>.ids.json`, seeded from `canister_ids.json`); there are no hardcoded ledger principals in the script. Because the hash doesn't prove the wiring, `getLedgers()` (anonymous-readable query, like `getStats`) exposes the configured `icvc_ledger`/`icp_ledger` principals so a deployment's ledger wiring is verifiable on-chain. Play uses the `ic` env (our ICRC-1 copies); real value uses the `prod` env (`canister_ids.json` `prod` key → real `m6xut`/`ryjl3`). When you add an anonymous-readable query, add it to the `inspect` message variant list too
...
- Pool accounting reads two numbers off on-chain truth (see "Phased funding model" in `README.md`): `total_icp_distributed` (from `getStats`) and the live pool balance (`icrc1_balance_of` on the ICP ledger, read directly by clients — `getStats` is a pure query and does NOT return it). The frontend renders these as "X distributed of (X + remaining) ICP in pool"; the denominator is computed in the browser. No `recordTranche` / `icp_committed` counter is maintained, so a DAO `icrc1_transfer` is the only step needed to update the displayed pool size. `getStats` is a query (anonymous-readable, no inter-canister call) so it can't be used to drain cycles
...
- **Concurrent interleaving of recovery methods** (the TOCTOU class: `retryRefund`/`forceRefund`/`sweepBurn`/`forceBurn` racing on the same id). The PocketIC Python harness has only blocking `update_call` (no `submit_call`), so the interleave can't be driven from the test by two ingress messages — but it **can** be forced deterministically with a **re-entrant mock ledger**: the mock, mid `icrc1_transfer`, calls back into the redemption canister for the same id *while the outer call is parked at its transfer await*. This is now regression-tested: `test_concurrent_recovery_guard_prevents_double_refund` (in `test_refund_recovery.py`) arms the mock to re-enter `forceRefund(id)` and asserts the ledger sees exactly ONE refund transfer (the per-id recovery guard short-circuits the second); it fails (count==2, double-refund) on the pre-guard wasm. The determinism comes from the await chain (the mock `await`s the re-entrant call before replying, so it must run inside the guard-held window) — this is **not** the flaky case. (The earlier, *abandoned* attempt was a different property: the `pendingBurns` append-vs-filter dropped-entry race, whose manifestation does depend on PocketIC round scheduling. The reconcile-against-current-`pendingBurns` design for that is still correct-by-construction and not separately regression-tested.)
