...
**Play deployment only** — the mainnet ICVC and ICP ledgers are our own copies of the ICRC-1 wasm, *not* the production tokens, and `faucet` is a testing-only affordance that would be removed before any real-value deployment.
...
- `Pure.mo` — pure helpers extracted from the actor for unit testing (`calculateIcpPayout`, `memoFromId`, `cooldownRemainingSeconds`, `intToNat64`)
...
- Exchange rate is in e8s: `5_753_022` means 0.05753022 ICP per 1 ICVC (the current fair-value-derived rate; see the H2 note below)
...
- Redemption canister is pre-funded with 800k ICP and 20M ICVC (for faucet) via `initial_balances` in ledger init args
- Faucet has a 10-second cooldown per principal (demo UX defence; not security)
- Faucet also honours the `paused` flag — pausing the canister halts both `redeem` and `faucet` (it does not stop `retryRefund`, which stays available for recovery)
...
- `seedData`, `closeSeeding`, `isSeedingClosed` no longer exist (removed once mainnet was seeded). Fresh local deploys start with an empty redemption history; use `faucet` + `redeem` to populate it during testing
...
- Tests pure helpers in `src/redemption/Pure.mo`: `calculateIcpPayout`, `memoFromId`, `cooldownRemainingSeconds`, `intToNat64`. Anything actor-stateful or async cannot live in `Pure.mo`.
...
| Financial correctness | faucet balance delta, redeem balance delta, `total_icvc_redeemed` increment, redeem burns ICVC (`total_icvc_burned` += amount, supply drops, canister balance unchanged), `sweepBurn` admin no-op |
...
- Deterministic time control for the faucet cooldown and lock TTL — **covered by PocketIC** (`test_faucet_cooldown`, `test_cooldown_boundary`).
