...
| `test_sequential_redeems.py` | **Stats math over many calls.** 5 sequential redeems, after each one every counter (`total_icvc_redeemed`, `total_icp_distributed`, `total_redemptions`) and the live pool balance (read via `icrc1_balance_of`) must match the cumulative expectation. Catches off-by-one drift. |
...
| `test_refund_recovery.py` | **Refund/burn retry recovery + the per-id recovery guard.** Uses the general `mock_ledger.mo` (runtime-toggleable `icrc1_transfer`) as both ledgers, failing then healing a transfer. Covers: `retryRefund` happy path (`#RefundPending` → refunded), `sweepBurn` flushing a failed inline burn, `getDedupKey` on a real stuck entry, `forceCloseInFlight` clearing one, and **`test_concurrent_recovery_guard_prevents_double_refund`**: the mock, armed, re-enters `forceRefund(id)` mid refund-transfer (while the outer call is parked holding the per-id guard) so the ledger must see exactly ONE refund transfer (it sees two on the pre-guard wasm — deterministic red/green). |
